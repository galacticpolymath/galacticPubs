...
drive_parse_name <- function(dribble){
  ...
  checkmate::assert(
  checkmate::check_class(dribble,"dribble")
  )
  ...
  part=ifelse(substr(tolower(nom_split[2]),1,1)=="p"|!grepl("-",nom_split[2],fixed=TRUE),  gsub("[a-zA-Z]","",nom_split[2]), NA)
  ...
  if(is.na(part)){
    ...
    remaining_str<-paste0(nom_split[3:length(nom_split)],collapse="")
  }else{
    grade_str<-nom_split[3]
    remaining_str<-paste0(nom_split[4:length(nom_split)],collapse="")
  }
  ...
  if(!is.na(delim_loc)){
    ...
    remaining_str<-paste0(substr(grade_str,delim_loc,nchar(grade_str)),remaining_str,collapse="")
    grade_str<-substr(grade_str,1,delim_loc-1)
  }
    #finally, get grade band by stripping out grade prefix
    grades<-gsub("[a-zA-Z]*(\\d.*)","\\1",grade_str,perl = TRUE,fixed = FALSE)
    ...
    is_presentation<-grepl(pattern=".*(prese?n?t?a?t?i?o?n?).*",x=remain)
    ...
    type=if(sum(type_tests)==0) NA else type_names[which(type_tests)]

    #Guess environment
    is_classroom<-grepl(pattern=".*(classroom).*",x=remain)
    is_remote<-grepl(pattern=".*(remote).*",x=remain)
    envir<-if(!is_classroom&!is_remote){NA}else{c("classroom","remote")[which(c(is_classroom,is_remote))]}
  ...
  checkmate::assert(
    ...
    checkmate::check_character(type,any.missing=FALSE),
    combine="and"
