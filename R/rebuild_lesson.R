...
#' @param stage do you want to stage assets that are rebuilt in the published/ folder for that lesson (prep to be published)? default=TRUE
...
#'
#'
rebuild_lesson <- function(shortName,lessons_dir,stage=TRUE,change_this=NULL){
  ...
  if(!dir.exists(lessons_dir)){
    ...
    update_list<-lapply(good_projects,function(WD){
      ...
      #2. batchCompile the lesson
      batchCompile(WD=WD,rebuild=TRUE)
