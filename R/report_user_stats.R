...
report_user_stats <- function(verbosity = 1,
  ...
  is_date_end <- lubridate::is.Date(end_date)
  #Try to coerce start date to date if not supplied
  if (!is_date_start) {
    start_date <- lubridate::ymd(start_date)
    if (is.na(start_date)) {
      stop("Inappropriate start date format. Use format 'yyyy-mm-dd'")
    }
  }

  #repeat logic for end_date
  if (!is_date_end) {
  ...
  users$createdAt <- ifelse(is.na(users$createdAt), "08-Dec-2024", users$createdAt)
  ...
  users2 <- users %>%
    dplyr::mutate(createdAt_date = lubridate::dmy(.data$createdAt)) %>%
    dplyr::mutate(Year = lubridate::year(.data$createdAt_date)) %>%
    dplyr::mutate(Month = sprintf("%02d", lubridate::month(.data$createdAt_date))) %>%
    dplyr::mutate(created_floor = lubridate::floor_date(.data$createdAt_date,"month"))


  #define theme
  report_theme <-
    galacticEdTools::theme_galactic(base.theme = "bw", font = "sans") +
    ggplot2::theme(
  ...
  user_growth <- users2 %>%
    ...
    dplyr::filter(.data$createdAt_date <= end_date) %>%
    ggplot2::ggplot(ggplot2::aes(x = .data$created_floor)) +
    report_theme +
  ...
  running_tot_students <- users2 %>%
    dplyr::group_by(.data$created_floor) %>%
    dplyr::summarise(total_class_size = sum(.data$classSize, na.rm = TRUE)) %>%
    dplyr::mutate(running_total = cumsum(.data$total_class_size))  %>%
    dplyr::filter(.data$created_floor >= start_date &
                    .data$created_floor <= end_date) %>%
    #plot
    ggplot2::ggplot(ggplot2::aes(
      x = .data$created_floor,
      y = .data$running_total,
      group = 1
    )) +
    report_theme +
    ggplot2::scale_x_date(
    ...
    ggplot2::labs(
      subtitle = paste0(
        "Total class size of GP Teacher Users (",
        scales::comma(sum(users2$classSize, na.rm = T)),
        ")"
      ),
      x = "",
      y = "Total class size"
  ...
  KPIs <- patchwork::wrap_plots(
    running_tot_students,
    user_growth,
    ncol = 1,
    nrow = 2,
    heights = c(0.7, 0.3)
  ) +
    patchwork::plot_annotation(title = "Galactic Polymath Impact Metrics", subtitle = "Data from teach.galacticpolymath.com")
  ...
  message(rep("-", 20))
  message(
    "User summary (galacticpolymath.com):\n- TOTAL: ",
    total,
    "\n- new in past week: ",
    new_7_days,
    "\n- new in past month: ",
