...
zassign_lsn_stats <- \(is_initialized, WD_git, fm, uinfo) {
  if (!is_initialized) {
    ...
    lsnStatuses <- purrr::map(1:nrow(uinfo), \(i) {
      ...
      if (!xi$lsnStatus %in% c("Live", "Beta")) {
        ...
        if (!is.character(updated_date) &
            !is.character(new_date) & is_empty(unit_release_date)) {
          sort_by_date <- NA
        } else{
          sort_by_date <-
            max(c(updated_date, new_date,unit_release_date), na.rm = TRUE) %>% as.character()
        }
      }
