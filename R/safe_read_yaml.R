#' safe_read_yaml
#'
#' Wrapper for [read_yaml][yaml::read_yaml()] that simplifies all null and missing data to NA or NULL. Creates front-matter.yml if not found
#'
...
#' @param standardize_missing logical; do you want all "",NULL,list(), etc. to be read in as NA using [is_empty()]?; default=FALSE
...
#' @param names_meaningful logical; passed to [is_empty()]; default=TRUE
#' @export
#
safe_read_yaml <- function(yaml_path = NULL,
                           ...
                           checkWD = TRUE,
                           auto_init = TRUE,
                           standardize_missing = FALSE,
                           missing_value = NA,
                           names_meaningful = TRUE) {
  ...
  stndrdize <- function(x, missing_value, names_meaningful=names_meaningful) {
    out <- lapply(1:length(x), function(i) {
      ...
      if (length(xi) > 1) {
        ...
        xi <- stndrdize(xi, missing_value = missing_value,names_meaningful = names_meaningful)
        if (unlist_after) {
          xi <- xi %>% unlist()
        }
      }
      ...
      if (is_empty(xi, names_meaningful = names_meaningful)) {
        if (is.null(missing_value)) {
          xi <- NULL
        } else{
          xi <- rep(missing_value, length(xi))
        }
      } else{
        xi
      }
  ...
  if (standardize_missing) {
    stndrdize(y, missing_value = missing_value,names_meaningful = names_meaningful)
  } else{
    y
  }
