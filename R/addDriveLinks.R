...
#' @export
#'
addDriveLinks<-function(shortTitle,linksFile="meta/teaching-materials.xlsx",envir=c("quickprep_feedback","remote","classroom")){
  ...
  #Extract filenames, links, etc. for all
    ...
    out.envir <- lapply(1:length(envir),function(k){
        ...
        #handle quickprep "environment"
        if(envir_k=="quickprep_feedback"){
          # out.qp<-lapply(1:nrow(pathDribble),function(i){
            #If the filename has "-part 1-" in it, extract parts, otherwise put an NA (i.e. for forms)
            qp_parts<-ifelse(grepl("^.*[-_].*[P|p][^\\d]*(\\d*).*[_-].*",pathDribble$name,perl=T),
            ...
            data.frame(envir=envir_k,parts=qp_parts,grades=NA,filename=pathDribble$name,filetype=qp_filetypes,gDriveLink=qp_links)
        ...
        #handle all remote and classroom environments differently
          ...
          out.grades<-lapply(1:nrow(gradeFolders),function(i){
            ...
            out.category<-lapply(c("presentations","handouts"),function(category){
              ...
              parts<-ifelse(grepl("^.*[-_].*[P|p][^\\d]*(\\d*).*[_-].*",currCatFiles$name,perl=T),
