...
#' @param use_wd_git if TRUE, will use the git root of the current working directory as the base path for relative paths. Makes no expectation that front-matter.yml is present or working (useful in combination with [fm_restore()] when YML is corrupted)
...
pick_lesson <- function(shared_drive = "s",
  ...
  if (use_wd_git) {
    wd_git <- get_wd_git()
    projects00 <- fs::dir_ls(fs::path(wd_git, "Lessons"), type = "directory")
    projects_sorted <- dplyr::tibble(
      CHOICE = 1:length(projects00),
      unit = basename(projects00),
      path = projects00
    )
  } else{
