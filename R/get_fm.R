...
#' @param standardize_NA logical; do you want all "",NULL,list(), etc. to be read in as NA using [is_empty()]? passed to [safe_read_yaml()]; default=FALSE
...
get_fm <-
  function(key = NULL,
    ...
    if (length(results)==1 & !is.list(results[[1]]) & !always_list) {
      unlist(results)
    } else{
      results
    }
