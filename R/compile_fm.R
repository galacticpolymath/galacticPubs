...
compile_fm <- \(WD = "?",
 ...
 if(upload){
  ...
  ver <-
    get_fm("Versions", WD = WD, standardize_NA = FALSE)[[1]] %>%
    dplyr::as_tibble() %>% dplyr::arrange(.data$ver_num)#ensure latest entry always at bottom
