...
y <- safe_read_yaml(WD = WD, eval.expr = TRUE)
...
server <- function(input, output, session) {
  ...
  output$compile <- renderUI({
    #prep stuff
    scriptFiles <-
      list.files(path = fs::path(WD, "scripts"), pattern = ".R")
  ...
  observe({
    isolate({
      #Save selections
      current_data <-
        prep_input(input, yaml_path, WD = WD)$current_data
  ...
  output$preview <- renderUI({
    current_data <-
      prep_input(input, yaml_path, vals$current_data, WD = WD)$current_data
  ...
  observe({
    ...
    vals$current_data <-
      overwrite_matching(safe_read_yaml(yaml_path,checkWD = FALSE), vals$current_data)
